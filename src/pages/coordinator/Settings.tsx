...
export default function CoordinatorSettings() {
  ...
  const [selectedVersion, setSelectedVersion] = useState<PPCVersion | null>(null);
  ...
  const handleSave = (data: Partial<PPCVersion>) => {
    const newVersion: PPCVersion = {
      ...
      version: data.version || `PPC ${new Date().getFullYear()}.${Math.ceil((new Date().getMonth() + 1) / 6)}`,
    ...
    toast({ title: 'PPC atualizado com sucesso! Nova versão criada.' });
  };
  ...
  return (
    <AppLayout breadcrumb={['Início', 'Configurações do Curso']}>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-primary">Configurações do Curso</h1>
          <p className="text-muted-foreground">Gerencie o PPC, regras de validação e comissões avaliadoras</p>
        </div>
        <Card>
          <CardContent className="p-6">
            <Tabs defaultValue="ppc">
              ...
              <TabsContent value="ppc" className="mt-6 space-y-6">
                <PPCCurrentForm currentVersion={currentVersion} onSave={handleSave} />
              </TabsContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedVersion.totalHours} horas</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Regra de Alocação</p>
                ...
                <div>
                  ...
                  <p className="font-medium">{formatDate(selectedVersion.effectiveDate)}</p>
                </div>
              </div>
              
              <div className="border-t pt-4">
                <p className="text-sm text-muted-foreground">Autor da Alteração</p>
              ...
              <div>
                ...
                <p className="font-medium">
                  {format(new Date(selectedVersion.createdAt), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                </p>
              </div>
            </div>
          )}
